...
The wave function of one electron in the Hamiltonian given in equation \ref{eq:Hamiltonian} is given by
...
where $H_{nx} \sqrt{\omega} x$ is the Hermite polynomials and A is the normalization constant, and $n_x$ and $n_y$ are the energy quantum numbers. If we denote $\psi_1$ the spatial wave function for the first electron, and $\psi_2$ the spatial wave function for the second electron, then the total spatial wave function for the two-particle system is $\phi_1 \phi_2$. As explained in section \ref{sec:Presentation_of_potential}, the spin wave functions are anti-symmetric, and therefore it is fine that the spatial wave function is symmetric.

For a one-electron system the \textcolor{red}{energy quantum numbers (??)} are 0, and the energy can be shown to take the value 
...
For the non-interacting case it is known that when using natural units, the ground state energy is 3 a.u.
