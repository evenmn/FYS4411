...
The system energy of a Gaussian-Binary RBM is given by \cite{Hinton}
\begin{equation}
...
In this project, we have experimented with one and two particles in a system, with different harmonic oscillator potential strengths $\omega$, and observed the results with and without the Coulomb repulsion. Another experiment which is usually done is to increase the number of particles in the trap. In our case, however, this does not make sense. As explained in section \ref{sec:Presentation_of_potential}, we assume that the trial wave function used is such that it can be split in a symmetric spatial and an antisymmetric spin part. In other terms, then one electron is in the "spin-up"-state, while the other is in "spin-down". This configuration can only hold two particles. According to Pauli's principle, two identical fermions cannot be found in the same state, and this system thus only makes physical sense for a maximum of two particles. 
