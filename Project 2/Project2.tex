...
\begin{table} [H]
	\caption{In this table we present average kinetic, interaction and potential energies for various $\omega$'s, along with the average distance between particles in the rightmost column. We used the brute-force Metropolis method with $dx=1.0$ and found the averages after convergence.  \vspace{2mm}}
...
Initially we calculate the energy for one particle in one dimension to ensure that the method works properly and we obtain an energy close to the analytical energy for brute-force Metropolis and Metropolis-Hasting, while Gibbs' sampling had a percent error of roughly 25\%. The reason for that is probably related to how we define the wave function. Since the wave function used in Gibbs' sampling is the square root of the probability distribution, this is farther from the true wave function. Nevertheless, we observe that brute-force Metropolis is slightly more accurate than the Metropolis/Hastings method. 

We can also see from \ref{tab:energies1P1D} that the calculated energies are really close to the exact energies for brute-force Metropolis and Metropolis-Hastings, and the variance decreases as we increase the number of iterations. 

When varying the number of hidden nodes, we saw that N=1 gave the lowest energy, 

We also observed that the convergence rapidness was heavily dependent on the learning rate, where a low learing rate gave a slow convergence. This is what we expect, since the learning rate defines how much we change the weights for each iteration. 
\vspace{3mm}

The onebody density, in figure \ref{fig:OB}, reproduces the analytical onebody density without interaction, and for the interacting case it gets broader. Since the electrons then are affected by repulsive force, they will have a larger separation distance and a broader distribution makes sense.
\vspace{3mm}
