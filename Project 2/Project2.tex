...
The neurons are arranged in layers in the neural network, one visible layer that receives the input, and up to several hidden layers. The layers are arranged such that the output values from the visible nodes are the input values of the hidden nodes. The nodes can also have bias values, that shift the output value $h$ with a certain number, and the nodes in different layers can be connected in different ways. An example of a neural network with multiple input and output nodes is shown in figure \ref{fig:neural_network}.
...
The idea behind machine learning is that the weights $W_i$, that decide how much a node puts emphazis on a given input, can be changed, and thus change the system's response to the same input. We will explain with the example from earlier, with huskies and the wolves: first, the program is shown pictures of wolves and huskies, and it is told the correct answer. The weights $W_i$ are then updated such that when a husky is shown, the output "husky" is generated, and then "wolves" for pictures of wolves. After a sufficiently long training period, the program's weights are optimalized for recognizing wolves and huskies. When shown a picture, the program should then by itself be able to determine whether it is a wolf or a husky that it sees. 
...
\begin{equation}
	...
	\phi_{n_x, n_y} = A H_{n_x} \sqrt{\omega} x H_{n_y} \sqrt{\omega} y e^{-\frac{\omega(x^2 + y^2)}{2}}
\end{equation}
where $H_{n_x} \sqrt{\omega} x$ are the Hermite polynomials and A is the normalization constant, and $n_x$ and $n_y$ are the principal quantum numbers. If we denote the spatial wave function for the first electron $\phi_1$, and $\phi_2$ for the spatial wave function for the second electron, then the total spatial wave function for the two-particle system is $\phi_1 \phi_2$. As explained in section \ref{sec:Presentation_of_potential}, the spin wave functions are antisymmetric, and therefore it is fine that the spatial wave function is symmetric.

We know that the energy of a harmonic oscillator in one dimension is given by

\begin{equation}
	\label{eq:HO_energy}
	E_n = \omega \Big(n + \frac{1}{2}\Big)
\end{equation}
where $n$ is the principal quantum number.

For an one-electron system in two dimensions the energy can be shown to take the value 
\begin{equation}
...
\begin{equation}
	H \ket{\phi_1 \phi_2} = \ket{H \phi_1 } \ket{\phi_2} + \ket{\phi_1} \ket{H \phi_2} = E_1 \ket{\phi_1 \phi_2} + E_2 \ket{\phi_1 \phi_2} = (E_1 + E_2) \ket{\phi_1 \phi_2}
\end{equation}
...
Numerically we solve this by dividing the area around a particle into bins. Thereafter we count the number of particles in each bin, and divid by the area to get the density. The area of bin $k$ is
\begin{equation}
A_k=\pi r_k^2-\pi r_{k-1}^2=2\pi r_kdr-\pi dr^2
\end{equation}
where $r_k$ is the outer radius of bin $k$ and $dr$ is the bin width. For a more thoroughout explanation, see \cite{Nordhagen}.
\par 
\vspace{3mm}
...
\end{equation}
for a particle in the groundstate of a harmonic oscillator potential with $A$ as the normalization constant.

\subsection{Virial theorem} \label{sec:Virial_theorem}
The virial theorem is an equation in mechanics that gives a relation between the time-averaged kinetic energy $\langle T \rangle$ and the forces acting on $N$ particles confined in a stable system. The theorem takes the following form:
...
\sigma =  \sqrt{\frac{1}{n-1} \sum_{k=1}^n (x_i - \langle x \rangle)^2}
\end{equation}

When giving the uncertainty, one writes $x_i \pm \sigma$, where $\sigma$ is so that $\approx 68 \%$ of the measured values lies within the interval $\pm \sigma$. \par 
\vspace{3mm}
...
Above, we described that the VMC method had to reject or accept proposed moves of the particles in the system. The task of handling this is often left up to the Metropolis algorithm. The Metropolis algorithm uses the theory that the probability of a system to transition from a state $i$ to state $j$ is given by
...
\end{equation}
where $\boldsymbol{r}$ are random numbers and $step$ is the steplength.
\par 
\vspace{3mm}
...
As an alternative to the Metropolis algorithm, one can use the Gibbs sampling method. In both the brute force method and Metropolis-Hastings, one has to account for that proposed particle moves can be rejected. This "wastes" one Monte Carlo cycle  and several flops, and no change is inflicted on the system. \par 
\vspace{3mm}
The Gibbs sampling algorithm differs from the Metropolis in several ways. In section \ref{sec:RBM} we wrote that we chose the trial wave function $\Psi = F(\boldsymbol{X},\boldsymbol{H})$, where $F$ is the joint probability distribution defined in equation \ref{eq:F_rbm}. In Gibbs sampling, we instead use the trial wave function $\Psi_T = \sqrt{F}$. 
\par 
\vspace{3mm}
...
As explained earlier, $\boldsymbol{X}$ are the positions of the particles, equal to the visible nodes, while $\boldsymbol{h}$ are the hidden nodes. $\boldsymbol{a}$ and $\boldsymbol{b}$ are the visible and hidden biases, and $\boldsymbol{W}$ are the weights on the nodes. The value $\sigma$ controls the width of the sampling distribution for the positions.\par 
\vspace{3mm} 
...
\vspace{3mm}
As we use a new trial wave function in Gibbs sampling, the expression for the local energy changes. The expression for the local energy using the Gibbs trial wave function is calculated in appendix B.
...
\end{equation}
where $\eta$ is the learning rate and $\alpha_i^+$ is the new value of $\alpha_i$. We need to do this for all the variational parameters, which are $\boldsymbol{W}$, $\boldsymbol{a}$ and $\boldsymbol{b}$. For our purposes it is convenient to vectorize the gradient such that $d\boldsymbol{a}$ and $d\boldsymbol{b}$ are vectors and $d\boldsymbol{W}$ is a matrix. The gradients to implement are
\begin{equation}
...
In this project we are using the blocking code produced by Marius Jonsson, see \cite{Jonsson}.
...
In order to keep the code structured, we have split the running of the program up in several files. The user only needs to interact with main.cpp, as all the parameters for running the machine learning project are set here. Examples are that the user can choose between brute force, Metropolis-Hastings og Gibbs sampling, and set the number of visible and hidden nodes, along with the number of iterations. main.cpp calls on the function GradientDescent in gradient\_descent.cpp, where the machine learning structure is implemented. The machine learning structure consists of the VMC-loop, where the nodes are updated, and the gradient descent method, where the weights are updated. GradientDescent uses the class WaveFunction in wavefunction.cpp to calculate the value of the wave function, along with the energy of the system. Depending on whether the user chose brute force, Metropolis-Hastings or Gibbs sampling, GradientDescent calls functions in either wavefunction.cpp, hastings\_tools.cpp or gibbs\_tools.cpp to update the positions and calculate the transition probabilities. The test functions are found in test.cpp, where both a test for checking the convergence of the energy is implemented, along with a function that tests if the local energy calculation is correct. The structure can be seen in figure \ref{fig:program_structure}.
...
We first study the energy of a non-interacting electron in one dimension, and in figure \ref{fig:energy1P1D} we compare the energies produced by the three VMC sampling methods as a function of number of iterations. 
 \begin{figure} [H]
 	\centering
 	\includegraphics[scale=0.8]{plots/energy_comparison.png}
 	\caption{Energy as a function of number of iterations up to 500 iterations for one particle in one dimension. Brute-Force (BF) Metropolis (blue line), Metropolis-Hastings (MH) (orange line), Gibbs (green line) and analytical value (dashed red line). All the samplings were conducted with MC=$2^{20}=1\,048\,576$, $\eta=0.01$, two hidden nodes, $\omega=1.0$ and $\sigma=1.0$. For BF we used steplength $dx=1.0$, while for MH the timestep was $dt=0.01$.}
...
\subsubsection{Changing learning rate and number of hidden nodes}
...
We observe that the optimal number of hidden nodes in this case is N=1. In figure \ref{fig:compare_etas} we compare the energy convergence for various $\eta$'s.  Initially all the weights and positions are set to 0.2, and again we use brute-force Metropolis with $dx=1.0$.
 ...
 \end{figure}
The smaller learning rate the slower it converges, and after 100 iterations both $\eta$=0.5 and $\eta$=0.05 are close to the exact value. 

\subsection{Energy calculations, with interaction}
We now go further to study the results from two iteracting electrons.
...
Again we play with the number of nodes and the learning rate
 ...
 \begin{figure} [H]
 	\centering
 	\includegraphics[scale=0.8]{plots/energy_compare_nodes_2P.png}
 	\caption{Energy as a function of number of iterations up to 1000 iterations for two particles in two dimensions. Brute-Force Metropolis was used for N$=1$ (blue line), N$=2$ (orange line), N$=4$ (green line) and N$=10$ (red line). All the samplings were conducted with MC=$2^{27}=134\,217\,728$, $\eta=0.01$, $dx=1.0$, $\omega=1.0$ and $\sigma=1.0$.}
 	\label{fig:energy_compare_nodes_2P}
 \end{figure}

 \begin{figure} [H]
 	\centering
 	\includegraphics[scale=0.8]{plots/energy_compare_eta_2P.png}
 	\caption{Energy as a function of number of iterations up to 1000 iterations for two particles in two dimensions. Brute-Force Metropolis was used for $\eta=0.01$ (blue line), 0.05 (orange line), 0.1 (green line) and $0.5$ (red line). All the samplings were conducted with MC=$2^{25}=33\,554\,432$, two hidden nodes, $dx=1.0$, $\omega=1.0$ and $\sigma=1.0$.}
 	\label{fig:energy_compare_eta_2P}
 \end{figure}

\subsection{Gibbs sampling}
We will also study how the results are affected by the choices of $N$, $\eta$ and $\sigma$ for Gibbs sampling. 

\subsubsection{Changing learning rate and number of hidden nodes}
In figure \ref{fig:energy_compare_eta_2P_gibbs} the energy is calculated as a function of number of iterations, for a various number of hidden nodes. We can see that $N=1$ gives the lower energy. Further, in figure \ref{fig:energy_compare_eta_2P_gibbs}, the energy is plotted as a function of number of iterations for different $\eta$'s. The higher learning rate, the faster it converges. 

 \begin{figure} [H]
 	\centering
 	\includegraphics[scale=0.8]{plots/energy_compare_nodes_2P_gibbs.png}
 	\caption{Energy as a function of number of iterations up to 1000 iterations for two particles in two dimensions. Gibbs sampling was used for N$=1$ (blue line), N$=2$ (orange line), N$=4$ (green line) and N$=10$ (red line). All the samplings were conducted with MC=$2^{20}=1\,048\,576$, $\eta=0.01$, $dx=1.0$, $\omega=1.0$ and $\sigma=1.0$.}
 	\label{fig:energy_compare_nodes_2P_gibbs}
 \end{figure}

  \begin{figure} [H]
 	\centering
 	\includegraphics[scale=0.8]{plots/energy_compare_eta_2P_gibbs.png}
 	\caption{Energy as a function of number of iterations up to 1000 iterations for two particles in two dimensions. Gibbs sampling was used for $\eta=0.01$ (blue line), 0.05 (orange line), 0.1 (green line) and $0.5$ (red line). All the samplings were conducted with MC=$2^{25}=33\,554\,432$, two hidden nodes, $dx=1.0$, $\omega=1.0$ and $\sigma=1.0$.}
 	\label{fig:energy_compare_eta_2P_gibbs}
 \end{figure}

\subsubsection{Changing the sampling width $\sigma$ used in Gibbs}
Finally, Gibbs sampling is investigated for different $\sigma$'s in figure \ref{fig:energy_compare_sigma_2P_gibbs}, and we observe that $\sigma=0.8$ gives the lowest energy, close to 3.0 a.u. 

  \begin{figure} [H]
 	\centering
 	\includegraphics[scale=0.8]{plots/energy_compare_sigma_2P_gibbs.png}
 	\caption{Energy as a function of number of iterations up to 1000 iterations for two particles in two dimensions. Gibbs sampling was used for $\sigma=0.5$ (blue line), 0.8 (orange line), 1.0 (green line), $1.2$ (red line) and $1.5$ (purple line). All the samplings were conducted with MC=$2^{20}=1\,048\,576$, two hidden nodes, $dx=1.0$, $\eta=0.5$ and $\omega=1.0$.}
 	\label{fig:energy_compare_sigma_2P_gibbs}
 \end{figure}
...
Initially we calculated the energy for one particle in one dimension to ensure that the method worked properly. Using equation \ref{eq:HO_energy} to calculate the ground state energy, we get that the energy for one particle in one dimension with $\omega=1.0$ should be $\frac{1}{2}$. The result of the calculation with the machine learning program with different sampling algorithms can be seen in figure \ref{fig:energy1P1D}. We see that both the  brute-force Metropolis and Metropolis-Hasting converges towards the analytical energy. Additionally, from table \ref{tab:energies1P1D}, we see that the variance decreases when the two methods converges towards $\frac{1}{2}$, as we increases the number of gradient descent cycles used in the minimalization. This is a good indication that our methods are approaching the correct value, even if we did not have an analytical answer to compare them to. The variance is calculated with the blocking method, presented in section \ref{sec:Blocking}, in order to account for the covariance between measured points.  The two methods yield nearly identical results, while we would expect Metropolis-Hastings to be a better method, due to the more advanced sampling routine presented in section \ref{sec:Metropolis-Hastings}. However, the difference might be too small to give an impact to this project, as we use few particles. Furthermore, we used a two-power number of Monte Carlo cycles for blocking. 
\par 
\vspace{3mm}

From figure \ref{fig:energy1P1D}, one can see that Gibbs sampling converges to a higher energy than the analytical value. The reason for that is probably related to how we define the wave function. For brute force and Metropolis-Hastings, we use the probability distribution defined in equation $\ref{eq:F_rbm}$ to represent the wave function, while the wave function used in Gibbs sampling is the square root of the probability distribution. It seems that the parameters chosen for this wave function by the machine learning program does not yield the ground state wave function, and thus the energy calculated is higher. This is in accordance with the variational principle; a worse guess on trial wave function will yield a upper bound estimate of the ground state energy $E$ that is larger than the true ground state energy $E_0$.

\par 
\vspace{3mm}
For the case where the Coulomb repulsion is turned on, the results of the convergence of the Metropolis algorithms and the Gibbs sampling method are shown in figure \ref{fig:energy2P2D} and table \ref{tab:energies2P2D}. Again, as described above, we see that the brute force and Metropolis-Hastings algorithms converge towards a value close to the exact value of $3$ a.u, while Gibbs sampling approaches a higher energy. The fraction of deviation between the calculated and analytical value is roughly the same for the interacting and non-interacting case for all three methods, which can be seen by comparing table \ref{tab:energies1P1D} and \ref{tab:energies2P2D}.

\par 
\vspace{3mm}
We varied the number of hidden nodes $N$ used in the machine learning program, as seen in figure \ref{fig:compare_nodes}, \ref{fig:energy_compare_nodes_2P} and \ref{fig:energy_compare_nodes_2P_gibbs} for without and with interaction with brute force, and with interaction with Gibbs respectively. We see generally that varying the number of hidden nodes changes the rapidness of the method's convergence. For few hidden nodes, the convergence of the energy seems to happen at a slower rate, while for many hidden nodes, it descends steeply towards the analytical energy. This can maybe be explained by when there are more free parameters that can be varied, the optimalization occurs faster. \par 
We also see in figure \ref{fig:compare_nodes} and \ref{fig:energy_compare_nodes_2P_gibbs} that we get better estimates of the ground state energy when using fewer hidden nodes. This can be explained, since more hidden nodes means more variables that has to be updated for each cycle, and thus we get a less precise answer. However, this is not the case in figure \ref{fig:energy_compare_nodes_2P}, where $N=10$ gives the lower estimate of the energy. This is probably due to the fact that the runs in this plot with different numbers of hidden nodes are forced to start at the same place, while we see that this is not the case in figure \ref{fig:compare_nodes} and \ref{fig:energy_compare_nodes_2P_gibbs}. 
\par 
\vspace{3mm}

Furthermore, we observed the effects of changing the learning rate $\eta$ used in the gradient descent method. The results are shown in figure \ref{fig:compare_etas}, \ref{fig:energy_compare_eta_2P} and \ref{fig:energy_compare_eta_2P_gibbs}. We see that the convergence rapidness is heavily dependent on the learning rate, where a low learning rate gives a slow convergence. This is what we expected, as the learning rate defines how much the weights $W$, $a$ and $b$ are updated in each gradient descent iteration, that is, how large steps the method takes for each iteration. Larger steps will lead to the method approaching the convergence more rapidly. As seen in this figure, if we limit the number of gradient descent iterations, then the choice of $\eta=0.01$ has not converged by the end of 100 minimalization iterations. A good choice of $\eta$ is thus necessary for the method to work optimally.
\par  
\vspace{3mm}

In figure \ref{fig:energy_compare_sigma_2P_gibbs}, we show the effects of changing the parameter $\sigma$ in the Gibbs sampling method, where $\sigma$ is the parameter deciding the width of the positions samping distribution, as seen in equation \ref{eq:gibbs_xsampling}. We can see that the energy the method converges towards is heavily dependent of the choice of $\sigma$. For $\sigma=1.0$, as is the default choice, we get convergence to a energy above the ground energy, while the choice of $\sigma=0.8$ gives converge towards 3 a.u., which is known to be the analytical ground state energy of the system. Since small changes in $\sigma$ gives vastly different results, it should probably have been implemented as a minimization parameter, so that the user does not have to vary it by hand in order to get optimal results
\par
\vspace{3mm}
Generally, from varying both the learning rate $\eta$, the number of hidden nodes $N$, and also the sampling width $\sigma$ used in the Gibbs algorithm, we see that the quality of the results we obtain is very dependent on the correct choices of these parameters. This is a drawback with this machine learning program; the user cannot necessarily depend on the program to give correct output on the first run, and if it seems the method gives the wrong output, it might simply be due to not optimal input parameters. Regarding $\sigma$, this can be solved, as explained above, by implementing it as a parameter that is optimized through the gradient descent method. Furthermore, one could have implemented tests to check what learning rate and number of hidden nodes that converges to energies closest to the known ground state, but this is not helpful if there is no analytical solution. In general, the user should be aware of this drawback with machine learning, and also be smart when chosing parameters to run the program with. 
\par 
\vspace{3mm}
The onebody density calculated for two particles in two dimensions with the Metropolis-Hastings method is plotted in figure \ref{fig:OB}. We are able to reproduce the analytical onebody density without interaction. When interaction is introduced, the distribution of particles becomes broader and lower. This is explained by the two electrons now being subject to a repulsive force which pushes them apart, so that the average distance between the electrons increases. This creates a broader onebody density distribution.

We also see that the graphs are slightly noisy near origin, although we run for 1e9 Monte Carlo cycles. That is because the area we divide by is small, and we therefore need a very large number of cycles to get a statistical descent number of particles in this bin. 
\par 
\vspace{3mm}

In table \ref{tab:average_energies}, we see that the estimates of the average distance between the two electrons $\langle |\boldsymbol{r}_i-\boldsymbol{r}_j|\rangle$ increases with increasing harmonic oscillator frequency $\omega$. This is as expected, as the higher $\omega$, the steeper the sides of the potential well, and thus the electrons are forced together. We also observe that the average distance for the same $\omega$ increases when the interaction is turned on, which as explained above, corresponds to the Coulomb repulsion forcing the electrons apart.
\par 
\vspace{3mm}

As explained in section \ref{sec:Virial_theorem}, the virial theorem holds for particles in stable systems, and the relation between the time-averaged kinetic and potential energy for potentials that are proportional to $r^n$ is shown in equation \ref{eq:virial_simple}. In the non-interacting case, the only potential affecting the particles is the harmonic oscillator potential, which is proportional to $r^2$. Thus equation \ref{eq:virial_simple} gives that the time-averaged kinetic and potential energies should be equal. From table \ref{tab:average_energies}, we see that we are able to reproduce this for two particles without interaction and $\omega=1.0$. However, for different values of $\omega$, it seems that the method becomes unstable, and we are no longer able to reproduce the virial theorem. If we look at the hermite functions, which are the analytical wave functions in a harmonic oscillator potential for ideal particles, the exponents are scaled with respect to $\omega$. The NQS wave function is not scaled in the same way, which may explain why the virial theorem only works for $\omega=1.0$.

In the interaction case, both the harmonic oscillator potential and the Coulomb interaction contributes to the total potential, and the total potential is thus no longer on the form $r^n$. Thus the simple relation between the time-averaged kinetic and potential energies in equation \ref{eq:virial_simple} no longer applies.
\par 
\vspace{3mm}
...
\par 
\vspace{3mm}
 An experiment which is usually done in these kinds of project, and as we did in \cite{Nordhagen}, is to increase the number of particles in the trap. In this case, however, this does not make sense. As explained in section \ref{sec:Presentation_of_potential}, we assume that the trial wave function used is such that it can be split in a symmetric spatial and an antisymmetric spin part. In other terms, then one electron is in the "spin-up"-state, while the other is in "spin-down". This configuration can only hold two particles. According to Pauli's principle, two identical fermions cannot be found in the same state, and this system thus only makes physical sense for a maximum of two particles. 
...
\par 
\vspace{3mm}
The choice of learning rate $\eta$ and number of hidden nodes $N$ used in the simulation affects the convergence rate and final results, where the convergence rate is higher for more hidden nodes and higher  $\eta$. The energy the method converges towards is closer for to the analytical energy for lower $N$ and higher $\eta$. The choice of $\sigma$ strongly affects the calculated results from the Gibbs sampling algorithm. 
\par 
\vspace{3mm}
The onebody density distribution obtained in this project matches the analytical calculation for the non-interacting case. The distribution becomes broader and lower when turning on the interaction, which matches the physical explaination that the repulsive Coulomb force forces the particles apart.
\par
\vspace{3mm}
...
The energy can be calculated by introducing a local energy, where the average local energy goes to the true energy with a sufficient amount of samples, as explained in section \ref{sec:Theory}. This local energy can be splitted up in a kinetic part, a part from the harmonic oscillator potential and an interacting part,
\begin{equation}
...
\subsection{Gradient for gradient descent method}
...
\section{Appendix C - Derivation of the wave function} \label{sec:appendix_C}
...
\begin{thebibliography}{}
	...
	\bibitem{Nordhagen}
	 ...
	 \bibitem{Project2}
	 V. Flugsrud and M. Hjorth-Jensen
	...
	\bibitem{Taut}
	 ...
	 \bibitem{Jonsson}
	 M. Jonsson, \emph{Captain Hook}, Program for blocking, \url{https://github.com/computative/block} downloaded April 2018. 
