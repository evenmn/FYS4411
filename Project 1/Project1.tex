...
In section \ref{sec:OB_theory} we presented a couple of ways computing the onebody density. Because both should give the same result, we selected the simplest one, which is the method with the bins. We conduct the investigations in elliptic traps, with 10 particles, 3 dimensions and 1e6 Monte Carlo cycles. With the optimal parameter $\alpha$ for the ground state wave function and for three different choices of $a$, the onebody density plots are found in figure (\ref{fig:ob0}).

\begin{figure} [H]
    \centering
    \includegraphics[scale=0.65]{images/ob.png}
    \caption{The onebody density of the bosonic system. $a=0$ is run with the optimal variational parameter $\alpha = 0.5$, while the $a=0.0043$ is run with $\alpha = 0.3450$. In addition the analytical onebody density without interaction ($a=0$) is plotted as a dashed line. As length is dimensionless, both the density $\rho$ and the length $r$ are dimensionless}
    \label{fig:ob0}
\end{figure}
We observe that the computed onebody density for the non-interacting case matches the analytical graph, but for small radii we get some noise. The onebody density for the interacting case has a lower peak, but seems broader than for $a=0$. The density approaches zero when the radius increases, and when $r\geq3$, both the densities are zero.
