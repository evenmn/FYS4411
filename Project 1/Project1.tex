...
\label{eq:Hamilton}
\end{equation}
with $V_{ext}$ as the external potential, which is the harmonic oscillator potential defined in equation \ref{eq:V_ext},
...
For the spherical trap situation we are left with the variational parameters $\alpha$ and $\beta$ only, but when we study an elliptical trap we still want to get rid of $\omega_z$. Since $\beta^2$ should be the factor in front of the z-coordinate when the Hamiltonian is dimensionless, \textcolor{red}{it can be proven that $\beta=\omega_z/\omega_{HO}$, see appendix C -> Dette er vel ikke riktig? $\gamma = \sqrt{\frac{m}{\hbar \omega_{HO}}}\omega_z a_{HO}$}. We end up with the Hamiltonian
\begin{equation}
...
From equation \ref{eq:WF_nointeract}, the trial wave function for one particle and one dimention is as follows:
