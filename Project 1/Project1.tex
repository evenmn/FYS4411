...
and $V_{int}$ as the interaction term. The interaction will in the first place be ignored, and is given later.

The trial wavefunction is on the form 
\begin{equation}
\Psi_T(\vec{r}_1, \vec{r}_2, ..., \vec{r}_N, \alpha, \beta)=\prod_i^Ng(\alpha, \beta, \vec{r}_i)\prod_{i<j}^Nf(a,r_{ij})
\label{WF}
\end{equation}
...
The most interesting and realistic case is when the interaction is included, so let us now include the interaction term, which is given by a so-called hard-sphere
\begin{equation}
